...
import { createCommandProcessor } from "./game_commanding.js";
...
function render() {
  ...
  state.ships.forEach((s, idx) => {
    ...
    const capacityLabel = state.currentScenario >= 3 && !s.utility
      ? ` | cap ${s.cargoCapacity || SHIP_CAPACITY_BY_ID[s.id] || 0}`
      : "";
...
const handleCommand = (...args) => commandProcessor.handleCommand(...args);
